        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="max-w-xl">
                    <section>
                        <header>
                            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ __('Update company') }}</h2>
                            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __("Update your company information") }}</p>
                        </header>
                        <form action="{{ route('companies.update', $viewData->id) }}" method="POST" class="mt-6 space-y-6">
                            @csrf
                            @method("PUT")
                            @foreach([
                                ['name' => 'code',    'title' => 'Code',    'value' => $viewData->code],
                                ['name' => 'name',    'title' => 'Name',    'value' => $viewData->name],
                                ['name' => 'address', 'title' => 'Address', 'value' => $viewData->address]
                            ] as $field)
                                <div>
                ...
                <div class="mt-10">
                    <section>
                        <header class="flex flex-row justify-between items-center">
                            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ __('Departments') }}</h2>
                            <a  class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150"
                                href="{{ route('departments.create', $viewData->id) }}">
                                <svg 
                                    class="w-5 h-5 mr-2"
                                    viewBox="0 0 32 32" 
                                    version="1.1" 
                        ...
                        <div class="mt-4 w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            <div class="grid grid-cols-4 flex flex-row text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                @foreach (['ID', 'Name', 'Code', 'Actions'] as $col)
                                    <div class="px-6 py-3 border-1 border-black">
                            ...
                            <div id="departments" class="mt-4 w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                <x-tree-view-table :elements="$viewData->departments" depth="1" />
                            </div>
                        </div>
                    </section>
                </div>
