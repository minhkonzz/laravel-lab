            ...
            <div class="relative overflow-x-auto shadow-md p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="pb-5 w-full flex flex-row justify-between items-center">
                    <form action="{{ route('tasks.filter') }}" class="flex flex-row" method="GET">
                        ...
                        @foreach ([
                            ['options' => $companies,  'title' => 'Company',  'inputName' => 'companies',  'selectedIds' => $selectedCompanies ?? []],
                            ['options' => $projects,   'title' => 'Project',  'inputName' => 'projects',   'selectedIds' => $selectedProjects ?? []],
                            ['options' => $priorities, 'title' => 'Priority', 'inputName' => 'priorities', 'selectedIds' => $selectedPriorities ?? []],
                            ['options' => $statuses,   'title' => 'Status',   'inputName' => 'statuses',   'selectedIds' => $selectedStatuses ?? []]
                        ] as $filter)
                            <div class="ml-2">
                                <x-input-label :for="$filter['inputName']" :value="__($filter['title'])" />
                                <x-dropdown-search-checkbox class="mt-1.5" :options="$filter['options']" :inputName="$filter['inputName']" :selectedIds="$filter['selectedIds']" />
                            </div>
                        @endforeach
                ...
                <table
                    ...
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>@foreach (['ID', 'Name', 'Start time', 'End time', 'Priority', 'Status', 'Actions'] as $col)
