    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <form action="{{ route('projects.update', $viewData->id) }}" method="POST" class="mt-6 space-y-6">
                    @csrf
                    @method("PUT")
                    <div class="max-w-xl">
                        <section>
                                        ...
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                            ...
                                            <td class="px-6 py-4">
                                                <label class="inline-flex items-center cursor-pointer" id="toggle-container-{{ $i }}">
                                                    ...
                                                    <div class="w-10 h-6 {{ in_array($person->id, $viewData->selectedPersonIds) ? 'bg-black' : 'bg-gray-300' }} rounded-full transition duration-300 relative">
                                                        <span class="w-4 h-4 bg-white rounded-full absolute top-1 left-1 transition-transform transform {{ in_array($person->id, $viewData->selectedPersonIds) ? 'translate-x-[16px]' : 'translate-x-0' }}"></span>
                                                    </div>
                                                </label>
                                            </td>   
                                        </tr>
...
<script>
    const personList = document.querySelector('#companyPersons tbody');

    function setupToggles() {
        for (let i = 0; i < personList.childElementCount; i++) {
            const container = document.getElementById(`toggle-container-${i}`);
